...
def IDT_VR(data, 
    ...
    while True:
        ...
        # print(init_time,fin_time,end_idx)
        ...
        for i in range(1, sub_data.shape[0]):
            ...
            frec_list.append(1/(t_int+1e-5)*100000)
        
        print(np.sum(np.array(frec_list) < frec_th), len(np.array(frec_list)))
        # break
        ...
        if np.sum(np.array(frec_list) < frec_th) == 0:
            ...
            output = list(map(compute_disp_angle, 
                                      zip([[head_mean_pos_x]*sub_data.shape[0]],
